...
class TestController extends Controller
{
    public function index(){
        ...
        $ranks = $this->rankSetKey(Rank::all());
        return $this->getRank($ranks, $data, auth()->user()->username);
    }
    ...
    // public function rankUserCount($ranks, $data, $username){
    //     $user = $data[$username]->referrals;

    //     $ranksCount = [];
    //     foreach ($ranks as $rank) {
    //         $ranksCount[$rank['id']] = 0;
    //     }

    //     foreach ($user as $usr) {
    //         $rankName = $data[$usr['username']]->rank->rank_name ?? null;
    //         if ($rankName && array_key_exists($rankName, $ranksCount)) {
    //             $ranksCount[$rankName]++;
    //         }
    //     }
    //     return $ranksCount;
