...
<style>
    ...
    .btn-secondary {
        ...
        border: none;
        background: #4eda92;
        background: -moz-linear-gradient(-45deg, #4eda92 0%, #1d945b 100%);
        background: -webkit-linear-gradient(-45deg, #4eda92 0%,#1d945b 100%);
        background: linear-gradient(135deg, #4eda92 0%,#1d945b 100%);
    ...
    <div class="row m-auto">
        <div class="col-12 mt-5">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-8 col-md-12 col-sm-12 col-12 border p-1">
                            <div class="row">
                                @forelse ($products as $item)
                                    @if ($item['item_type'] == 'product')
                                        @php
                                            $product = product($item['item_id']);
                                            $product_stock = agent_product_stock($product->id, auth()->user()->username, auth()->user()->id );
                                        @endphp
                                        @if ($product_stock > 0)
                                            <div class="col-lg-3 col-md-4 col-sm-6 col-12 py-2">
                                                <div class="product">
                                                    <a href="{{ route('product', ['id' => $product->id, 'slug' => $product->slug]) }}" target="_blank">
                                                        <div class="product-header">
                                                            <img class="img-fluid" src="{{ $product->thumbnail ? asset('uploads/product/' . $product->thumbnail) : asset('frontend/img/small/1.jpg') }}"
                                                            alt="{{ $product->slug }}">
                                                        </div>
                                                        <div class="product-body">
                                                            <h5>{{ substr($product->title, 0, 20) }}</h5>
                                                            <h6>{{ substr($product->sub_title, 0, 20) }}</h6>
                                                        </div>
                                                        <div class="product-footer">
                                                            <a href="javascript:">
                                                                <button type="button" onclick="addCart({{ $product->id }}, 1, 'product');" class="btn btn-secondary btn-sm float-right">Add To Cart</button>
                                                            </a>
                                                            @if ($product->offer_price == 0 || $product->offer_price == null)
                                                                <p class="offer-price mb-0">{{ currency()['symble'] }} {{ $product->price }}</p>
                                                            @else
                                                                <p class="offer-price mb-0">{{ currency()['symble'] }} {{ $product->offer_price }} <i class="fa fa-tags"></i> <span class="regular-price">{{ currency()['symble'] }} {{ $product->price }}</span></p>
                                                            @endif

                                                            <p class="offer-price mb-0">Stock : {{ $product_stock }}</p>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                    ...
                                    @if ($item['item_type'] == 'package')
                                        @php
                                            $package = package($item['item_id']);
                                            $package_stock = agent_package_stock($package->id, auth()->user()->username, auth()->user()->id);
                                        @endphp
                                        @if ($package_stock > 0)
                                            <div class="col-lg-3 col-md-4 col-sm-6 col-12 py-2">
                                                <div class="product">
                                                    <a href="javascript::" target="_blank">
                                                        ...
                                                        <div class="product-footer">
                                                            <a href="javascript:">
                                                                <button type="button" onclick="addCart({{ $package->id }}, 1, 'package');" class="btn btn-secondary btn-sm float-right">Add To Cart</button>
                                                            </a>
                                                            <p class="offer-price mb-0">{{ currency()['symble'] }} {{ $package->price }}</p>

                                                            <p class="offer-price mb-0">Stock : {{ $package_stock }}</p>
                                ...
                                @empty
                                    <div class="col-md-12 pmb-3 text-center">No Products...</div>
                                @endforelse
                            </div>

                        </div>
                        <div class="col-lg-4 col-md-12 col-sm-12 col-12 border py-3">
                            <div class="cart-sidebar">
                                <div class="cart-sidebar-header">
                                    <h5> My Cart <span class="text-success itemCount2">(5 item)</span> <a data-toggle="offcanvas" class="float-right" href="javascript:"><i class="mdi mdi-close"></i></a></h5>
                                </div>
                                ...
                                <div class="cart-sidebar-footer">
                                    ...
                                    <form action="" method="POST">
                                        @csrf
                                        <input type="hidden" name="cookie_id" id="agent_cookie_id" value="{{ request()->cookie('agent_cookie_id') }}">
...
@push('js')
    <script>
        ...
        function addCart(product_id, qty, type) {
            var qty = $('.quantity').val() || qty;
            url = "{{ route('addCart.agent.package.repurchase.store') }}";
            $.ajax({
                type: "POST",
                dataType: "json",
                data: {
        ...
        function addCartData() {
            url = "{{ route('addCart.agent.get.data') }}";
            $.ajax({
                type: "GET",
                dataType: "json",
                url: url,
                    ...
                    if(data.length != 0){
                        $.each(data, function(index, value) {
                            ...
                            if((value.product.offer_price == 0) || (value.product.offer_price == null)){
                                priceText = '<p class="offer-price mb-0">{{ currency()['symble'] }} '+ value.product.price +'/p>';
                                product_price = value.product.price;
                            }
                            else{
                                priceText = '<p class="offer-price mb-0">{{ currency()['symble'] }} '+ value.product.offer_price +' <i class="fa fa-tags"></i> <span class="regular-price">{{ currency()['symble'] }} '+ value.product.price +'</span></p>';
                            ...
                            html += '<a class="float-right remove-cart" href="javascript:" onclick="removeCartData('+ value.product.id +');"><i class="fa fa-times"></i></a>';

                            html += '<img class="img-fluid" src="'+ proImage +'" alt="">';

                            html += '<h5><a href="#">'+ value.product.title +'</a></h5>';

                            html += ' <h6 class="text-warning"><strong><span class="text-warning fa fa-coins"></span></strong> '+ value.product.point +' Point</h6>';
        ...
        function updateCookie(product_id, qty){
            url = "{{ route('addCart.agent.update') }}";
            $.ajax({
                type: "POST",
                dataType: "json",
                data: {
        ...
        function removeCartData(product_id){
            url = "{{ route('addCart.agent.destroy', ':product_id') }}";
