                                        ...
                                        <div class="product">
                                            <a href="">
                                                ...
                                                <div class="product-body">
                                                    ...
                                                    <h6 class="mt-0">My Stock : {{ agent_package_stock($package->id, auth()->user()->username, auth()->user()->id) }}</h6>
                                                </div>
                                            </a>
                                            <div>
                                                <button class="btn btn-secondary btn-sm form-control" onclick="addCart({{ $package->id }}, 1, {{ $client->id }});">Add To Cart</button>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <div class="col-md-4">
                            <div class="cart-sidebar">
                                ...
                                <div class="cart-sidebar-footer">
                                    ...
                                    <form action="{{ route('user.package.client.purchase.store') }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="cookie_id" id="agent_cookie_id" value="{{ request()->cookie('package_cookie_id') }}">
...
@push('js')
    <script>
        ...
        function addCart(product_id, qty, client_id) {
            var rowCount = $('#increment').closest("tr").length;

            if(rowCount < 1){
                var qty = $('.quantity').val() || qty;
                url = "{{ route('addCart.agent.package.store') }}";
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    data: {
                ...
                Swal.fire({
                    title: 'Sorry',
                    text: "You Can Not Purchase Multiple Package.",
                    icon: 'warning',
                    showCancelButton: false,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
        ...
        function addCartData() {
            url = "{{ route('addCart.agent.package.get.data', $client->id) }}";
            $.ajax({
                type: "GET",
                dataType: "json",
                url: url,
                success: function(data) {
                    ...
                    if(data.length != 0){
                        $.each(data, function(index, value) {
                            ...
                            html += ' <h6 class="text-warning"><strong><span class="text-warning fa fa-coins"></span></strong> '+ value.package.point +' Point</h6>';
        ...
        function updateCookie(product_id, qty){
            var url = "{{ route('addCart.agent.package.update') }}";
        ...
        function removeCartData(product_id){
            var client_id = "{{ $client->id }}";
            var url = "{{ route('addCart.agent.package.destroy', [':product_id', ':client_id']) }}";
            url = url.replace(':product_id', product_id).replace(':client_id', client_id);
            $.ajax({
                type: "GET",
                url: url,
                success: function(data) {
