                            ...
                            <div class="row">
                                @forelse ($products as $item)
                                    <div class="col-lg-3 col-md-4 col-sm-6 col-12 py-2">
                                        <div class="product">
                                            <a href="{{ route('product', ['id' => $item->id, 'slug' => $item->slug]) }}" target="_blank">
                                                <div class="product-header">
                                                    {{-- <span class="badge badge-success">50% OFF</span> --}}
                                                    <img class="img-fluid" src="{{ $item->thumbnail ? asset('uploads/product/' . $item->thumbnail) : asset('frontend/img/small/1.jpg') }}"
                                                    alt="{{ $item->slug }}">
                                                    {{-- <span class="veg text-success mdi mdi-circle"></span> --}}
                                                </div>
                                                <div class="product-body">
                                                    <h5>{{ substr($item->title, 0, 20) }}</h5>
                                                    <h6>{{ substr($item->sub_title, 0, 20) }}</h6>
                                                </div>
                                                <div class="product-footer">
                                                    <a href="javascript:">
                                                        <button type="button" onclick="addCart({{ $item->id }}, 1);" class="btn btn-secondary btn-sm float-right">Add To Cart</button>
                                                    </a>
                                                    @if ($item->offer_price == 0 || $item->offer_price == null)
                                                        <p class="offer-price mb-0">{{ currency()['symble'] }} {{ $item->price }}</p>
                                                    @else
                                                        <p class="offer-price mb-0">{{ currency()['symble'] }} {{ $item->offer_price }} <i class="fa fa-tags"></i> <span class="regular-price">{{ currency()['symble'] }} {{ $item->price }}</span></p>
                                                    @endif
